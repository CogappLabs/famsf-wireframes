...
// ConstituentDocument has no role field. Augment locally for the Role facet.
interface ArtistRow extends ConstituentDocument {
	role: string;
}
...
const FACETS: FacetDef[] = [
	...
	},
	{
		...
		label: "Dates active",
		getValue: (a) => {
			const c = activeCentury(a.display_date ?? "");
					...
					<Container>
						...
						{filtered.length === 0 ? (
							...
							<ul className="mt-6 grid grid-cols-1 gap-px bg-gray-200 sm:grid-cols-2 lg:grid-cols-3">
								{filtered.map((artist) => {
									const slug = slugByName[artist.name.toLowerCase()];
									const href = slug
										? `/constituents/sample/${slug}`
										: "/constituents/sample";
									return (
											...
											>
												...
												<div className="min-w-0 flex-1">
													...
													<p className="mt-0.5 font-mono text-label text-gray-500">
														{[artist.nationality, artist.display_date]
															.filter(Boolean)
															.join(", ") || "–"}
													</p>
													<p className="mt-0.5 font-mono text-label text-gray-400">
														{artist.role}
														{artist.object_count > 0 &&
															` · ${artist.object_count} ${t("artistSearch.worksLabel")}`}
													</p>
												</div>
											</Link>
										</li>
